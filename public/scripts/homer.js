...
requirejs([
  ...
  $.fn['animatePanel'] = function () {
    ...
    setTimeout(function () {
      $('.stagger').css('animation', '')
        .removeClass(effect).removeClass('animated-panel').removeClass('stagger');
    }, animateTime)
  ...
  if($(".select2").length) {
    requirejs(['select2-ru'], function() {
      $(".select2").select2();
    });
  }

  if($(".masked").length) {
    requirejs(['jquery-maskedinput'], function() {
      $(".masked.phone_number").mask("+7 (999) 999-9999");
    });
  }

  if($(".datepicker").length) {
    requirejs(['bootstrap-datepicker-ru'], function() {
...
requirejs([
  ...
  var sendTokenToServer = function (token) {
    $.post('/admin/chat/set_admin_token', {
      token,
